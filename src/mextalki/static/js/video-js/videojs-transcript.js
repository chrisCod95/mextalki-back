...
var utils = (function (plugin) {
  return {
    ...
    createEl: function (elementName, classSuffix) {
      ...
      element.className = plugin.prefix + classSuffix;
      if (elementName === 'select' && plugin.settings.enableBoostrap){
        element.className = `${element.className} form-control`
      }
      if (elementName === 'div' && classSuffix === '-body' && plugin.settings.enableBoostrap){
        element.className = `${element.className} container mt-2 border p-2`
      }
      return element;
    },
    extend: function(obj) {
...
var widget = function (plugin) {
  ...
  var createTranscriptBody = function (track) {
    ...
    let body = utils.createEl('div', '-body');
    let line, i;
    ...
    for (i = 0; i < lines.length; i++) {
      ...
      if (time > begin && time < end) {
        if (!line.classList.contains('is-active')) { // don't update if it hasn't changed
          ...
          if(plugin.settings.enableBoostrap){
            line.classList.add('alert');
            line.classList.add('alert-primary');
        ...
        if(plugin.settings.enableBoostrap){
            ...
            line.classList.remove('alert-primary');
            line.classList.remove('m-0');
          }
      }
    }
  };
